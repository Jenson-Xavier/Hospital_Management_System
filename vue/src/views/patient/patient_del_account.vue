...
// 定义注册数据
const usernameDelData = ref({
    verifyCode: '',
})

const oldPwdDelData = ref({
    oldPassword: '',
})
...
// 修改密码
// 更新密码
import { delMyAccountService } from '@/api/patient';
...
const updatePwd = async () => {
    ...
    if (selectedTabName.value == "oldPassword") {
        ...
        mode = "password";
        result = await delMyAccountService({
            oldPassword: oldPwdDelData.value.oldPassword,
            mode: mode
        })
    }
        ...
        result = await delMyAccountService({
            verifyCode: usernameDelData.value.verifyCode,
            mode: mode
        })
    }
    if (result.code == 0) {
        ElMessage.success("已成功注销账户");
        ...
        <el-card class="getBackPwd-card">
            ...
            <el-tabs v-model="selectedTabName" type="border-card" class="demo-tabs" stretch="true"
                ...
                <el-tab-pane label="原密码方式身份核验" name="oldPassword">
                    <!-- 表单 -->
                    <el-form class="form" :label-position="labelPosition" label-width="auto" :model="oldPwdDelData"
                        ...
                        <el-form-item label="原密码" :label-position="itemLabelPosition" prop="oldPassword">
                            <el-input v-model="oldPwdDelData.oldPassword" placeholder="请输入原密码" clearable
                                type="password" show-password />
                        </el-form-item>

                        <el-form-item>
                            <el-button class="button" type="primary" auto-insert-space @click="dialog_goto_del">
                                确认注销
                            </el-button>
                        </el-form-item>
                ...
                <el-tab-pane label="验证码方式身份核验" name="verifyCode">
                    ...
                    <!-- 表单 -->
                    <el-form class="form" :label-position="labelPosition" label-width="auto" :model="usernameDelData"
                        ...
                        <el-form-item label="验证码" :label-position="itemLabelPosition" prop="verifyCode">
                            <el-input class="input" v-model="usernameDelData.verifyCode" placeholder="请输入验证码"
                                type="password" show-password>
