...
const checkRePwd = (rule, value, callback) => {
    if (selectedTabName.value == "phone") {
        ...
        } else if (value != PhoneRegisterData.value.password) {
        ...
        if (value == '') {
            callback(new Error("请再次输入密码"));
        } else if (value != EmailRegisterData.value.password) {
            callback(new Error("两次密码输入不一致"));
        } else {
            callback();
        }
...
// 调用后台接口 完成注册
import { patientRegisterService, testTmpService } from '@/api/patient.js'
const register = async () => {
    if (selectedTabName.value == "phone") {
        // 表单准备提交进行校验
        await formRef_phone.value.validate().catch(err => {
            ElMessage.error('注册失败(前端 手机))')
            throw err
        })
    } 
    else if (selectedTabName.value == "email") {
    ...
    let result;
    if (selectedTabName.value == "phone") {
        const params = PhoneRegisterData.value;
        params["register_mode"] = "phone";
        result = await patientRegisterService(params);
    } else if (selectedTabName.value == "email") {
        const params = EmailRegisterData.value;
        params["register_mode"] = "email";
        result = await patientRegisterService(params);
    }
    if (result.code == 0) {
...
const sendVerificationCode = async () => {
    ...
    if (selectedTabName.value == "email") {
        // 调发送邮箱接口
        let result;
        result = await sendEmailService(EmailRegisterData.value);
        if (result.code == 0) {
    ...
    <el-row class="register-container">
        ...
        <el-card class="register-card">
            ...
            <el-row class="subtitle">
                ...
                <el-col :span="22">
                    <el-text class="subtitle">患者用户注册</el-text>
                </el-col>
            </el-row>
            ...
            <el-tabs v-model="selectedTabName" type="border-card" class="demo-tabs" stretch="true"
                ...
                <el-tab-pane label="手机号注册" name="phone">
                    <!-- 表单 -->
                    <el-form class="form" :label-position="labelPosition" label-width="auto" :model="PhoneRegisterData"
                        ...
                        <el-form-item label="用户名" :label-position="itemLabelPosition" prop="username">
                            <el-input v-model="PhoneRegisterData.username" placeholder="手机号" clearable/>
                        </el-form-item>

                        <el-form-item label="验证码" :label-position="itemLabelPosition" prop="verifyCode">
                            <el-input class="input" v-model="PhoneRegisterData.verifyCode" placeholder="请输入验证码"
                        ...
                        <el-form-item label="设置密码" :label-position="itemLabelPosition" prop="password">
                            <el-input v-model="PhoneRegisterData.password" placeholder="请设置密码" type="password" show-password/>
                        </el-form-item>

                        <el-form-item label="确认密码" :label-position="itemLabelPosition" prop="rePassword">
                            <el-input v-model="PhoneRegisterData.rePassword" placeholder="请再次输入密码" type="password" show-password/>
                        </el-form-item>
                ...
                <!-- 邮箱号注册 -->
                <el-tab-pane label="邮箱号注册" name="email">

                    <!-- 表单 -->
                    <el-form class="form" :label-position="labelPosition" label-width="auto" :model="EmailRegisterData"
                        :rules="emailRules" ref="formRef_email" style="max-width: 600px">

                        <el-form-item label="用户名" :label-position="itemLabelPosition" prop="username">
                            <el-input v-model="EmailRegisterData.username" placeholder="邮箱号" clearable/>
                        </el-form-item>

                        <el-form-item label="验证码" class="input" :label-position="itemLabelPosition" prop="verifyCode">
                            <el-input v-model="EmailRegisterData.verifyCode" placeholder="请输入验证码" type="password" show-password>
                        ...
                        <el-form-item label="设置密码" :label-position="itemLabelPosition" prop="password">
                            <el-input v-model="EmailRegisterData.password" placeholder="请设置密码" type="password" show-password/>
                        </el-form-item>

                        <el-form-item label="确认密码" :label-position="itemLabelPosition" prop="rePassword">
                            <el-input v-model="EmailRegisterData.rePassword" placeholder="请再次输入密码" type="password" show-password/>
                        </el-form-item>
